...
import 'package:first/views/budget_setup_screen.dart';
import 'package:first/models/budget_model.dart';
import 'package:first/views/budget_tracker_screen.dart'; // Добавили импорт

Future<void> main() async {
  WidgetsFlutterBinding.ensureInitialized();
  final budgetModel = await BudgetModel.loadModel(); // Загружаем модель
  final budgetViewModel = BudgetViewModel(budgetModel); // Создаем ViewModel

  runApp(
    ChangeNotifierProvider.value( // Используем ChangeNotifierProvider.value
      value: budgetViewModel, // Предоставляем ViewModel
      child: const MyApp(),
    ),
  );
}
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      routes: {
        '/budget_tracker': (context) => const BudgetTrackerScreen(),
      },
    );
  }
}
